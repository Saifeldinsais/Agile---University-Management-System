/**
 * Recreate Communication Tables (without staff_entity foreign key)
 * Run this script to fix the table structure
 */
...
async function recreateTables() {
    console.log('Recreating Student-Staff Communication tables...\n');

    try {
        // Drop existing tables if they exist (in correct order due to foreign keys)
        await pool.query('DROP TABLE IF EXISTS message_attachment');
        await pool.query('DROP TABLE IF EXISTS academic_guidance');
        await pool.query('DROP TABLE IF EXISTS meeting_request');
        await pool.query('DROP TABLE IF EXISTS student_staff_message');
        await pool.query('DROP TABLE IF EXISTS student_staff_conversation');
        console.log('✓ Dropped existing tables');

        // 1. Create conversation table (no foreign keys to allow any entity IDs)
        await pool.query(`
            CREATE TABLE student_staff_conversation (
        ...
        await pool.query(`
            CREATE TABLE student_staff_message (
        ...
        await pool.query(`
            CREATE TABLE academic_guidance (
        ...
        console.log('\n✅ Communication tables recreated successfully!');
        console.log('Note: All user IDs now reference the entities table (no staff_entity dependency)');
        process.exit(0);
